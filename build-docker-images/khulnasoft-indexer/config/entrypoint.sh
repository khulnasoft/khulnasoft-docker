...
export USER=khulnasoft-indexer
export INSTALLATION_DIR=/usr/share/khulnasoft-indexer
...
# is idempotent.
source /usr/share/khulnasoft-indexer/bin/opensearch-env-from-file
...
if [[ -f bin/opensearch-users ]]; then
  ...
  if [[ -n "$INDEXER_PASSWORD" ]]; then
    [[ -f /usr/share/khulnasoft-indexer/opensearch.keystore ]] || (run_as_other_user_if_needed opensearch-keystore create)
...
if [[ "$(id -u)" == "0" ]]; then
  ...
  if [[ -n "$TAKE_FILE_OWNERSHIP" ]]; then
    chown -R 1000:0 /usr/share/khulnasoft-indexer/{data,logs}
  fi
fi


#if [[ "$DISCOVERY" == "single-node" ]] && [[ ! -f "/var/lib/khulnasoft-indexer/.flag" ]]; then
  # run securityadmin.sh for single node with CACERT, CERT and KEY parameter
#  nohup /securityadmin.sh &
#  touch "/var/lib/khulnasoft-indexer/.flag"
#fi

run_as_other_user_if_needed /usr/share/khulnasoft-indexer/bin/opensearch <<<"$KEYSTORE_PASSWORD"